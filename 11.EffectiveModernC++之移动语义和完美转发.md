...
int main() {
  ...
  LogAndAdd(idx1);
  short idx2 = 1;
  LogAndAdd(idx2);  // 这里会匹配到 void LogAndAdd(T&& name)，并不会匹配到 void LogAndAdd(int idx)，所以会报错
}
```
...
int main() {
  ...
  short b = 2;
  Sample sample1(a);
  Sample sample2(b);  // 报错：调用Sample(T&& test)，参数无法匹配std::forward<T>
...
> 什么是`tag dispatch`？下面有介绍

上面的方法已经展示了传递`const T&`以及按值传递都不支持完美转发。如果在这方面有明确需要的话，有什么方式呢？
